{{-- <header class="bg-white dark:bg-gray-800 w-full z-50 fixed" id="header">
    <x-layout.hamburger />
    
    <nav class="w-full h-28 flex items-center px-12 py-0 md:py-8 sm:px-4 sm:!py-4 md:h-auto" id="navbar">
            ...
            <div class="flex flex-1 items-center !m-0 md:block">
                ...
                <div class="ml-auto block md:hidden md:ml-0 md:mt-10" id="mobile-menu">
                    <ul class="flex md:block md:space-x-0">
                        @foreach ($navbarItems as $navItem)
                            <x-layout.navbar-item :href="$navItem['href']"
                                                  :icon="$navItem['icon']"
                                                  :label="$navItem['label']" />
                        @endforeach
                    </ul>
...
<header class="bg-white dark:bg-gray-800 w-full z-50 fixed">
    <nav id="navbar" class="w-full h-28 flex items-center px-12 py-0 md:py-8 sm:px-4 sm:!py-4 md:h-auto">
        <div class="flex flex-1 justify-between items-center md:block md:justify-start md:space-x-10">
            ...
            <div class="flex flex-1 items-center !m-0 md:block">
                ...
                <div class="ml-auto block md:hidden md:ml-0 md:mt-10" id="mobile_navbar_list">
                    <ul class="flex md:block md:space-x-0">
                        @foreach ($navbarItems as $navItem)
                            <x-layout.navbar-item :href="$navItem['href']">{{ $navItem['label'] }}</x-layout.navbar-item>
                        @endforeach
