...
<x-util.card {{ $attributes->class( 'w-auto bg-transparent shadow-none justify-evenly' ) }}>
    ...
    <x-util.link-btn links="{{ $link }}" {{ $attributes->class([ 'bg-white text-black dark:bg-dark-600 dark:text-white mt-0 md:mt-7 focus:bg-minsk-900 hover:bg-minsk-900 focus:text-white hover:text-white' ]) }}></x-util.link-btn>
</x-util.card>
<x-util.card {{ $attributes->class( 'w-auto bg-transparent shadow-none justify-evenly' ) }}>
